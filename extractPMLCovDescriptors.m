function allDescriptors=extractPMLCovDescriptor(path, decimateValue)
...
allDescriptors = zeros(floor((TotalFrames-1)/decimateValue)+1, DescriptorLength);
...
while(hasFrame(video))
    ...
    if ~mod((numberOfFrame-1), decimateValue)
        ...
        allDescriptors((numberOfFrame-1)/decimateValue+1,:) = PMLDescriptors;
